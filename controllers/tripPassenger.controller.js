const { TripPassenger, PassengerCarCompany, Trip, PriceHeader, PriceLine, ImageVehicle, User, Comment, Station, Seats, Vehicles, sequelize, Point, Rate, Ticket, TimePoint } = require("../models");
...
const getAllTripPassenger = async (req, res) => {
	...
	try {
		if (tripId) {
			tripPassengerList = await TripPassenger.findAll({
				where: {
					tripId,
				},
		...
		if (tripPassengerList.length > 0) {
			...
			arrayResult = await Promise.all(tripPassengerList.map(async (tripPassenger) => {

				const priceHeader = await PriceHeader.findOne({
					where: {
						startDate: { [Op.lte]: new Date(tripPassenger.trip.startTime) },
						endDate: { [Op.gte]: new Date(tripPassenger.trip.startTime) },
						status: true
					}
				})
				...
				const price = await PriceLine.findOne({
					where: {
				...
				return {
					tripPassenger,
					priceTrip: price?.price | 0
				}

			}))
		}

		return res.status(200).send(arrayResult);
