...
import { getPromotionHeader, updatePromtionHeader } from "../../redux/actions/promotionHeaderAction";
...
export default function AdminPromotionLine() {
	const history = useHistory();
	const dispatch = useDispatch();
	const {id} = useParams();

	const {promotionHeader} = useSelector((state) => state.promotionHeader);
	...
	const formik = useFormik({
		...
		onSubmit: (values) => {
			console.log(values);
			dispatch(updatePromtionHeader(id, values));
		},
	});
	...
	useEffect(() => {
		if (id) {
			dispatch(getPromotionHeader(id));
	...
	useEffect(() => {
		if (promotionHeader) {
			formik.setErrors({});
			formik.setFieldValue("title", promotionHeader.title);
			formik.setFieldValue("status", promotionHeader.status);
			formik.setFieldValue("startDate", moment(promotionHeader.startDate).format("YYYY/MM/DD"));
			formik.setFieldValue("endDate", moment(promotionHeader.endDate).format("YYYY/MM/DD"));
		}
	}, [promotionHeader]);
