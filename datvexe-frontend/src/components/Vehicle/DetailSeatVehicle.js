...
import { useDispatch, useSelector } from "react-redux";
import { MinusCircleOutlined, PlusOutlined, FormOutlined, DeleteOutlined } from "@ant-design/icons";
import { Form, Input, Button, Space, message, Popconfirm, Popover } from "antd";
import { bookingSeatAction, bookingTicketAction } from "../../redux/actions/bookingAction";
import Wheel from "../Booking/Seat/Wheel";
import Seat from "../Booking/Seat/Seat";
import SeatBooked from "../Booking/Seat/SeatBooked";
import { getDetailVehicleAction } from "../../redux/actions/vehicleAction";
import { CreateSeatVehicleAction, DeleteSeatAction } from "../../redux/actions/SeatAction";
import { OPEN_DRAWER } from "../../redux/types/DrawerTypes";
import EditSeat from "../Edit/EditSeat";
import "../../Sass/css/Vehicle.css";
...
export default function DetailSeatVehicle(props) {
    ...
    const renderSeat = (floors) => {
        return vehicleDetail.seatVehicle.map((item, index) => {
            ...
            if (item.floor === floors) {
                return (
                    <Popover title={`Ghế: ${item.name}, Giá: ${item.price.toLocaleString()} VNĐ`} content={content} placement="top" key={index}>
    ...
    const renderFloor = () => {
        ...
        for (let index = 1; index <= vehicleDetail.numberFloors; index++) {
            arr.push(
                <div className="coach-container" key={index}>
                    <span>Tầng {index}</span>
                    <div className="coach">
                        <table>
                            <tbody>
    ...
    // Handle form submission and seat creation logic
    const onFinish = (values) => {
		console.log("Received values of form:", values);
		const isNormalType = vehicleDetail.type === "normal";
	
		// Default seat counts
		const defaultSeatCounts = {
			normal: 40,
			"normal-bed": 40,
			luxury: 32,
			limousine: 24,
		...
		// Creating seats based on vehicle type
		let listSeat = [];
		const seatCount = defaultSeatCounts[vehicleDetail.type] || 0;
	
		for (let i = 0; i < seatCount; i++) {
			listSeat.push({
				name: `Ghế ${i + 1}`, // Tên ghế tự động
				price: values.ListSeatAdd[0]?.price || 0, // Giá từ form
				floor: isNormalType ? 1 : (i < seatCount / 2 ? 1 : 2), // Logic tầng
				type: isNormalType ? "seat" : "bed", // Logic loại ghế
				status: "chưa đặt",
			});
		}
	
		if (vehicleDetail.seatVehicle.length + listSeat.length <= 40) {
			dispatch(CreateSeatVehicleAction(vehicleDetail.id, listSeat));
		} else {
			message.error("Thêm ghế xe thất bại. Đã đạt số lượng tối đa.");
		}
	};
    ...
    return (
        <>
            <div className="flex justify-center items-center">
                <div className="list-seat flex" style={{ width: "80%" }}>
                    <div className="seat-groups">
                        <div className="note font-bold" style={{ fontSize: "14px" }}>Chú thích</div>
                        <div className="seat-info">
                            <div className="seat-thumbnail">
                                <Seat />
                            </div>
    ...
    {(fields, { add, remove }) => (
        <>
            {fields.map(({ key, name, ...restField }) => (
                <Space key={key} style={{ display: "flex", marginBottom: 8 }} align="baseline">
                    <Form.Item {...restField} name={[name, "price"]} rules={[{ required: true, message: "Nhập giá ghế" }]}>
                        <Input placeholder="Giá Ghế" type="number" min={50000} />
                    </Form.Item>
                    <Form.Item {...restField} name={[name, "type"]} initialValue={vehicleDetail.type === "normal" ? "seat" : "bed"}>
                        <Input disabled />
                    </Form.Item>
                    <MinusCircleOutlined onClick={() => remove(name)} />
                </Space>
